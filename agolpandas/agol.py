...
def load_config(config_file='.agolcfg'):
    ...
    if not os.path.exists(config_file):
        config_file = os.path.join(os.path.expanduser("~"), '.agolcfg')
        if not os.path.exists(config_file):
            raise Exception('No .agolcfg file found')

    config = ConfigParser.ConfigParser()
    config.read(config_file)
    return dict(config.items('agol'))

def generate_token(username, password, referer=None):
    '''Obtain token from ArcGIS Online
